...
const Charts = dynamic(() => import('@/components/TradeCharts'), { ssr: false, loading: () => <ChartSkeleton /> })
...
export default function LogPage() {
  ...
  const violationLoss = closed
    .filter(t => t.ruleViolation && (t.pnl ?? 0) < 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <KPI label="通算損益" value={formatCurrency(totalPnl)} color={totalPnl >= 0 ? 'green' : 'red'} />
        <KPI label="勝率" value={closed.length > 0 ? `${winRate.toFixed(1)}%` : '—'}
          sub={`${wins}勝 ${closed.length - wins}敗`} />
        <KPI label="クローズ済" value={String(closed.length)} sub="件" />
        <KPI label="オープン" value={String(open.length)} sub="件" />
        <KPI
          label="ルール違反損失"
          value={violations > 0 ? formatCurrency(violationLoss) : '¥0'}
          sub={`${violations}件`}
      ...
      {/* Tabs */}
      <div className="flex gap-1 border-b border-[#1e1e30]">
        {([['charts', 'パフォーマンスグラフ'], ['trades', 'トレード履歴']] as const).map(([k, label]) => (
          <button
            key={k}
            onClick={() => setTab(k)}
        ...
        <div>
          {closed.length === 0 ? (
            <div className="border border-[#1e1e30] rounded-lg p-10 text-center">
              <p className="text-[#374151] text-sm">まだクローズしたトレードがありません</p>
              <p className="text-xs text-[#374151] mt-1">決済を記録するとグラフが表示されます</p>
            </div>
          ) : (
            <Charts trades={closed} />
          )}
        </div>
      )}
                    ...
                    <tr key={t.id} className="border-b border-[#1e1e30] hover:bg-[#0f0f1a] group transition-colors">
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          {t.ruleViolation && <span className="text-[10px] text-[#ef4444]" title="ルール違反">⚠</span>}
                        </div>
                        <p className="text-[10px] text-[#374151]">
                          {format(parseISO(t.entryDate), 'M/d', { locale: ja })}
                          {t.exitDate && ` → ${format(parseISO(t.exitDate), 'M/d', { locale: ja })}`}
                        </p>
                      </td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5 mono text-[#e2e8f0]">{t.lots.toLocaleString()}</td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          {t.result === 'OPEN' && (
                            <button onClick={() => startClose(t)} className="text-[10px] text-[#3b82f6] hover:text-blue-400">
                              決済
                            </button>
                          )}
                          <button onClick={() => removeTrade(t.id)} className="text-[10px] text-[#ef4444]/60 hover:text-[#ef4444]">
                            削除
                          </button>
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#0f0f1a] border border-[#1e1e30] rounded-xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="space-y-3">
              <Field label="決済価格 *">
                <input type="number" className="input" placeholder="決済した価格"
                  value={closeForm.exitPrice}
                  onChange={e => setCloseForm(f => ({ ...f, exitPrice: e.target.value }))} />
              </Field>
              <Field label="結果">
                <select className="input" value={closeForm.result}
              ...
              <div className="border border-[#1e1e30] rounded-lg p-3 space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={closeForm.ruleViolation}
                    onChange={e => setCloseForm(f => ({ ...f, ruleViolation: e.target.checked }))}
                    className="accent-[#ef4444]" />
                  <span className="text-xs text-[#e2e8f0]">ルール違反があった</span>
                </label>
                {closeForm.ruleViolation && (
                  <input className="input" placeholder="違反内容（例: 損切りラインを下抜けても持ち続けた）"
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button onClick={submitClose} disabled={!closeForm.exitPrice}
...
function KPI({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: 'green' | 'red' }) {
...
function ChartSkeleton() {
  return (
    <div className="grid md:grid-cols-2 gap-4">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="border border-[#1e1e30] bg-[#0f0f1a] rounded-xl p-4 h-52 animate-pulse" />
      ))}
    </div>
  )
}
